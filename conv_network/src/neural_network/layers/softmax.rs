...
use ndarray::{array, s, stack, Array, Array1, Array2, Array3, Axis,Ix2, Zip, ArrayD};
...
impl Layer for Softmax {
    ...
    */
    fn backward(&mut self, x: &ArrayD<f64>) -> ArrayD<f64> {
        ...
        stack(Axis(0), temp.as_slice()).unwrap().into_dyn()
    }
...
mod tests {
    ...
    fn it_should_give_correct_softmax() {
        ...
        let left2 = s.forward(&b.into_dyn());
        assert_eq!(
            left.into_dimensionality::<Ix2>().unwrap(),
            array![[0.09003057317038046, 0.6652409557748218, 0.24472847105479764]]
        );
        assert_eq!(
            left2.into_dimensionality::<Ix2>().unwrap(),
    ...
    fn it_should_give_correct_softmax_derivative() {
        ...
        let mut s = Softmax::init();
        let forward_call = s.forward(&a.into_dyn());
        let left = s.derivative().into_dimensionality::<Ix3>().unwrap();
        let right = array![
            [
                [
                    0.10354869134943266,
    ...
    fn it_should_give_correct_softmax_backward() {
        ...
        let mut s = Softmax::init();
        let forward_call = s.forward(&a.into_dyn());
        let left = s.backward(&temp_grad.into_dyn());
        assert_eq!(
            forward_call.into_dimensionality::<Ix2>().unwrap(),
            array![
                [
                    0.11731042782619835,
                    0.8668133321973347,
